...
\begin{document}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}
			...
			\State lastStage,bestweight = PlaceStage($G_{s},I[0],NULL$)
			\State tc = tc + bestweight
			\State s=s+1
			\State oc = oc + bestweight
			\While{There are more stages to place}
			\State ComputeGs($G,C[s]$)
			\State lastStage,bestweight = PlaceStage($G_{s},I[s],lastStage$)\\
			\#Calculate optimum by considering only a single stage
			\State lastStage,theoreticalOptimum = PlaceStage($G_{s},I[s],NULL$)
			\State s=s+1
			\State tc = tc + bestweight
			\State oc = oc + theoreticalOptimum
			\EndWhile
	...
	\end{algorithm}
	\begin{algorithm}[H]
		\caption{Place a single stage of the pipeline}
		\begin{algorithmic}
